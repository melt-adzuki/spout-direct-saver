...
internal sealed class RealtimePackedNvencWriter : IAsyncDisposable
{
    ...
    public RealtimePackedNvencWriter(
        uint width,
        uint height,
        double frameRate,
        string outputPath,
        ...
        int queueCapacity = 8)
    {
        ...
        var startInfo = new ProcessStartInfo
        {
            FileName = "ffmpeg",
            Arguments =
                $"-y -f rawvideo -pixel_format {pixelFormat.ToFfmpegPixelFormat()} -video_size {width}x{height} -framerate {frameRate:0.###} -i - -an -filter_complex \"[0:v]split=2[rgb][alpha];[alpha]alphaextract,format=gray,format=bgra[aout];[rgb]format=bgra[rout];[rout][aout]hstack=inputs=2[packed]\" -map \"[packed]\" -c:v hevc_nvenc -preset:v p1 -tune:v ll -rc:v vbr -cq:v 21 -b:v 0 -g:v {gop} -pix_fmt:v yuv420p -profile:v main \"{outputPath}\"",
        ...
        _writeTask = Task.Factory.StartNew(
            static state =>
            {
                var writer = (RealtimePackedNvencWriter)state!;
    ...
    public void QueueFrame(PixelBufferLease pixelData, int repeatCount)
    {
        ...
        {
            pixelData.Dispose();
            throw new InvalidOperationException("Realtime packed writer は既に完了しています。");
        }
    ...
    public async Task CompleteAsync(CancellationToken cancellationToken)
    {
        ...
        if (_process.ExitCode != 0)
        {
            ...
            throw new InvalidOperationException($"FFmpeg の packed alpha 書き出しに失敗しました。\n{details.Trim()}");
        }
    }
